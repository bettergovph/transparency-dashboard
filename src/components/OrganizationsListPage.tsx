...
const OrganizationsListPage = () => {
  ...
  const loadOrganizations = async (letter: string, search: string) => {
    ...
    try {
      ...
      const result = await index.search(searchQuery, {
        limit: 10000,
        attributesToRetrieve: ['organization_name', 'count', 'total', 'start_date', 'end_date'],
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="w-auto md:w-7xl max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex-1">
        <div className="mb-6">
            ...
            <>
              ...
              <div className="block lg:hidden divide-y divide-gray-200">
                {organizations.map((org, index) => (
                  <div key={org.name} className="px-3 py-4 hover:bg-gray-50 transition-colors">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      <div>
                        ...
                        <span className="ml-1 font-mono text-gray-900">{org.count.toLocaleString()}</span>
                      </div>
                      <div className="text-right">
                        <span className="text-gray-500">Period:</span>
                        <span className="ml-1 text-gray-600">{formatDateRange(org.startDate, org.endDate)}</span>
                      </div>
                      <div className="col-span-2 text-right">
                        <span className="text-gray-500">Total:</span>
                        <span className="ml-1 font-mono text-green-600 font-semibold">₱{org.total.toLocaleString()}</span>
                      </div>
                    </div>
                  </div>
                ))}
