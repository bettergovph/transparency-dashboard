          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6 max-w-[1800px] mx-auto">
            ...
            <Card>
              <CardHeader className="pb-3">
                ...
                <CardTitle className={`text-2xl flex items-center gap-2 ${yoyChange > 0 ? 'text-green-600' : yoyChange < 0 ? 'text-red-600' : 'text-gray-600'
                  }`}>
                  <TrendingUp className={`h-6 w-6 ${yoyChange < 0 ? 'rotate-180' : ''
                    }`} />
                      ...
                      {filteredRegions.map((region, index) => {
                        ...
                        return (
                          <tr key={region.region} className="hover:bg-gray-50 transition-colors">
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-right">
                              {regionYoyChange !== 0 ? (
                                <span className={`text-sm font-semibold flex items-center justify-end gap-1 ${regionYoyChange > 0 ? 'text-green-600' : 'text-red-600'
                                  }`}>
                                  <TrendingUp className={`h-4 w-4 ${regionYoyChange < 0 ? 'rotate-180' : ''
                                    }`} />
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          ...
                          <div className="bg-gray-50 rounded-lg p-2">
                            ...
                            {regionYoyChange !== 0 ? (
                              <p className={`text-sm font-bold flex items-center gap-1 ${regionYoyChange > 0 ? 'text-green-600' : 'text-red-600'
                                }`}>
                                <TrendingUp className={`h-3 w-3 ${regionYoyChange < 0 ? 'rotate-180' : ''
                                  }`} />
                                {regionYoyChange > 0 ? '+' : ''}{regionYoyChange.toFixed(1)}%
                              </p>
                            ) : (
