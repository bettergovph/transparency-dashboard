...
const EnhancedSearchInterface: React.FC<EnhancedSearchInterfaceProps> = ({
  ...
  const isRootPath = location.pathname === '/procurement'
  ...
  useEffect(() => {
    const delayDebounceFn = setTimeout(() => {
      ...
      if (query.trim() || selectedAreas.length > 0 || selectedAwardees.length > 0 || selectedOrganizations.length > 0 || selectedCategory !== 'all') {
        performSearch()
      } else {
        setResults([])
      }
