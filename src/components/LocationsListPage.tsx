  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* Chart */}
        {showChart && !loading && locations.length > 0 && (
          <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">
              Top {Math.min(20, locations.length)} Locations by Contract Count
            </h3>
            <ResponsiveContainer width="100%" height={400}>
              <BarChart data={locations.slice(0, 20)} layout="horizontal">
                ...
                <Bar dataKey="count" name="Contracts">
                  {locations.slice(0, 20).map((entry, index) => (
                    <Cell key={`cell-${index}`} fill={`hsl(${160 + index * 5}, 60%, ${45 + index}%)`} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
